...
const TOTAL_STEPS = 7 // verify + 6 steps

// Step 1 options — relationship status
const STATUSES = [
...
const INTIMATE_INTERESTS = [
  ...
  "Фетиш", "Полиамория", "Свингерство", "Танtra и медленное", "Без ограничений",
]
...
export function RegisterModal({ isOpen, onClose }: RegisterModalProps) {
  const [step, setStep] = useState(0) // 0 = verify, 1-6 = onboarding
  ...
  const toggleEveryday = (item: string) => {
    setEverydaySelected((prev) =>
      prev.includes(item) ? prev.filter((i) => i !== item) : [...prev, item]
    )
  }

  const toggleIntimate = (item: string) => {
    setIntimateSelected((prev) =>
      prev.includes(item) ? prev.filter((i) => i !== item) : [...prev, item]
    )
  }
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4">
          <div>
            ...
            {step > 0 && (
              <p className="text-xs text-gray-500 mt-0.5">Шаг {step} из 6</p>
            )}
          </div>
        ...
        <div className="px-6 pb-6 overflow-y-auto max-h-[70vh]">
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-2">
                {(["email", "phone"] as const).map((m) => (
                  <button
                    key={m}
                    onClick={() => { setVerifyMethod(m); setCodeSent(false); setVerifyValue("") }}
                    className={cn(
                      "flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium border transition-all",
                      verifyMethod === m
                        ? "text-white border-pink-500 bg-pink-500/15"
                        : "text-gray-400 border-gray-700 bg-transparent hover:border-gray-500"
                    )}
                  >
              ...
              <div className="space-y-3">
                <Label className="text-gray-300 text-sm">
                  {verifyMethod === "email" ? "Твой email" : "Номер телефона"}
                </Label>
                <div className="flex gap-2">
                  <Input
                    value={verifyValue}
                  ...
                  <Button
                    ...
                    className="shrink-0 text-white border-none"
              ...
              {codeSent && (
                <Button
                  ...
                  style={{ background: "linear-gradient(90deg,#ec4899,#a855f7)" }}
                >
                  Подтвердить и продолжить <ChevronRight className="ml-1 h-4 w-4" />
                </Button>
              )}
            </div>
          )}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <Input value={profile.age} onChange={(e) => setProfile({ ...profile, age: e.target.value })}
                    ...
                    className="bg-gray-900 border-gray-700 text-white focus:border-pink-500 border-pink-500/40"
                    placeholder="Только 18+" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {photos.map((src, idx) => (
                  <div key={idx} className="relative aspect-square rounded-xl overflow-hidden border border-gray-700">
                    ...
                    {idx === 0 && (
                      <span className="absolute bottom-1 left-1 text-xs bg-pink-500/80 text-white px-1.5 py-0.5 rounded-full">Главное</span>
                    )}
                  </div>
                ))}
                {photos.length < 6 && (
          ...
          {step === 4 && (
            <div className="space-y-5">
                ...
                {EVERYDAY_INTERESTS.map((item) => (
                  <button key={item} onClick={() => toggleEveryday(item)}
                    className={cn(
                      ...
                      everydaySelected.includes(item)
              ...
              {everydaySelected.length > 0 && (
                <p className="text-xs text-gray-500">Выбрано: {everydaySelected.length}</p>
              )}
            </div>
          )}

          {/* ── STEP 5: Intimate интересы ── */}
          {step === 5 && (
            <div className="space-y-5">
              <div>
              ...
              <div className="flex flex-wrap gap-2">
                {INTIMATE_INTERESTS.map((item) => (
                  <button key={item} onClick={() => toggleIntimate(item)}
                    className={cn(
                      "px-3 py-1.5 rounded-full text-sm border transition-all",
                      intimateSelected.includes(item)
              ...
              {intimateSelected.length > 0 && (
                <p className="text-xs text-gray-500">Выбрано: {intimateSelected.length}</p>
              )}
              <p className="text-xs text-gray-600">
                Эти данные видны только совместимым пользователям и только если ты разрешишь это в настройках.
              </p>
            </div>
          )}

          {/* ── STEP 6: Возможности приложения ── */}
          {step === 6 && (
            <div className="space-y-5">
              <div>
